  ...
  persist(
    (set, get) => ({
      ...
      addItem: async (item) => {
        ...
        try {
          if (!cartId) {
            const result = await createShopifyCart({ ...item, lineId: null } as CartItemInput);
            if (result) {
              set({
            ...
            const result = await addLineToShopifyCart(cartId, { ...item, lineId: null } as CartItemInput);
            if (result.success) {
              const currentItems = get().items;
